...
test.describe.skip('Admin Tagging Flow - Legacy/Fallback', () => {
    ...
    test.beforeAll(async () => {
        ...
        const fakeStorageUrl = 'https://firebasestorage.googleapis.com/v0/b/bucket/o/test_image.jpg?alt=media';
        ...
        await db.collection('image_detections').doc(detectionDocId).set({
            imageUrl: fakeStorageUrl,
            detectedCookies: [
                { x: 50, y: 50, width: 10, height: 10, confidence: 0.9, polygon: [] }
            ]
        });
    });
