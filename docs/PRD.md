...
- **AI Detection**:
  - Uses Google Cloud Vision API to detect cookies.
...
- **Firebase Functions**:
  - `processCookieImage` (in `index.ts`): Handles Cloud Vision API calls upon image upload to `uploads/`.
  - Secures the API access and handles data formatting.

### 5.3 AI Integration

- **Google Cloud Vision API**:
  - Service: Object Localization (AI).
  - Function: Object detection in images (returning bounding boxes).
