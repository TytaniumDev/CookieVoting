...
async function importData() {
    ...
    try {
        ...
        if (!existsSync(emulatorDataDir)) {
            throw new Error(`Export directory does not exist: ${emulatorDataDir}\nRun 'npm run emulators:export:manual' first to export data.`);
        }
                ...
                try {
                    ...
                    try {
                        ...
                        if (hasEmailPassword && userData.email) {
                            ...
                            console.log(`  🔄 Updated password for existing user: ${userData.email} (password: test123456)`);
                        } else {
                            console.log(`  ⏭️  User ${userData.email || userData.uid} already exists, skipping...`);
                        }
                    } catch (error) {
                    ...
                    console.error(`  ❌ Error importing user ${userData.email || userData.uid}:`, error.message);
                }
            }
