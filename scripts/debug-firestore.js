...
async function debugFirestore() {
    ...
    try {
        ...
        for (const eventDoc of eventsSnapshot.docs) {
            ...
            console.log(`      Created: ${eventData.createdAt ? new Date(eventData.createdAt).toISOString() : 'unknown'}`);

            // Check categories
            console.log(`\n   🏷️  Categories:`);
            const categoriesSnapshot = await db.collection('events').doc(eventId).collection('categories').get();
        ...
        if (useEmulator) {
            ...
            if (!testEventsSnapshot.empty) {
                console.log('   ⚠️  Found events in test_events collection - code might be using wrong collection!');
            }
        }
