...
const steps = [
    ...
    { name: 'Integration Tests', command: 'npm', args: ['run', 'test:integration'], continueOnError: true },
...
for (const step of steps) {
    ...
    if (result.status !== 0) {
        if (step.continueOnError) {
            console.warn(`⚠️  ${step.name} failed (exit code ${result.status}) but marked as non-fatal. Continuing...`);
        } else {
