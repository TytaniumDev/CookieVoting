 ...
 */
export function exportToCSV(event: VoteEvent, categories: Category[], votes: UserVote[]): string {
  const lines: string[] = [];
  ...
  votes.forEach((userVote) => {
    ...
    Object.entries(userVote.votes).forEach(([categoryId, cookieNumber]) => {
      ...
      lines.push(
        `${userVote.userId},${timestamp},"${category?.name || categoryId}",${cookieNumber}`,
      );
    });
  });
 ...
 */
export function exportToJSON(event: VoteEvent, categories: Category[], votes: UserVote[]): string {
  const data = {
    event: {
      id: event.id,
      name: event.name,
