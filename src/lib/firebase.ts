...
if (!hasValidConfig) {
  console.warn(
    'Firebase configuration is missing required values. Firebase services will not be available.',
  );
...
if (useEmulator) {
  try {
    ...
    if (!errorMessage.includes('already been initialized')) {
      ...
      console.warn(
        '   To use emulators, set VITE_USE_EMULATOR=true and run: npm run emulators:start',
      );
