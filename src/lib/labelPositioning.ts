...
export function calculateSmartLabelPositions(
  ...
  const wouldOverlap = (top1: number, left1: number, top2: number, left2: number): boolean => {
    const verticalOverlap = !(
      top1 + LABEL_HEIGHT_PCT < top2 - MIN_SPACING_PCT ||
      top1 > top2 + LABEL_HEIGHT_PCT + MIN_SPACING_PCT
    );
  ...
  items.forEach((item, index) => {
    ...
    if (nearbyLabels.length > 0) {
      ...
      const stillOverlaps = positions.some((p) =>
        wouldOverlap(bottomTop, proposedLeft, p.top, p.left),
      );
      ...
      if (stillOverlaps) {
        ...
        const altOverlaps = positions.some((p) =>
          wouldOverlap(bottomTop, altProposedLeft, p.top, p.left),
        );
