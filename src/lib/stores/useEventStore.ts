...
export const useEventStore = create<EventState>((set, get) => ({
    ...
    updateCategoryOrder: async (eventId: string, categoryId: string, newOrder: number) => {
        ...
        try {
            ...
            set(state => ({
                categories: state.categories.map(c => c.id === categoryId ? { ...c, order: newOrder } : c)
            }));
        } catch (error) {
    ...
    updateResultsAvailableTime: async (eventId: string, time: number | null) => {
        ...
        try {
            ...
            set(state => ({
                activeEvent: state.activeEvent ? { ...state.activeEvent, resultsAvailableTime: time || undefined } : null,
                loading: false
            }));
        } catch (error) {
