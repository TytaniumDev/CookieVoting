...
1.  **Separation of Concerns (Raw vs. Semantic)**:
    ...
    - **Solution**: `useImageStore` handles the _technical_ layer (ML bounding boxes). `useCookieStore` handles the _game_ layer (This box = "Ryan's Cookie"). This allows re-running detection without deleting votes.
...
- **Key Concept**: It cares about _pixels_, not _players_.
