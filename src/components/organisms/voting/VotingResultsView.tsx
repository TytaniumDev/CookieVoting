...
const ResultSlide = ({
    ...
    const rankedScores = sortedScores.map((score, i, arr) => {
        ...
        const uniqueHigherVotes = new Set(arr.slice(0, i).map(s => s.votes).filter(v => v > score.votes));
                    ...
                    renderTopLeft={({ index: cookieIndex }) => {
                        ...
                        const score = rankedScores.find(s => {
                            const distance = Math.sqrt(
                                Math.pow(detected.x - s.cookie.x, 2) +
                                Math.pow(detected.y - s.cookie.y, 2)
                            );
                            return distance < 5;
                        });
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.footer}>
                <button
                    onClick={currentIndex < results.length - 1 ? handleNext : () => window.location.href = '/'}
