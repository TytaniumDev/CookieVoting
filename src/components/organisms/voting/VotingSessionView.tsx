            ...
            <div className={styles.imageContainer}>
                <CookieViewer
                    ...
                    renderCenter={({ index, bounds }) => {
                        ...
                        return (
                            <>
                                {isSelected && <span className={styles.sparkle} style={{ position: 'absolute', transform: 'translate(-50%, -50%)', pointerEvents: 'none' }}>✨</span>}
...
export const VotingSessionView = ({ categories: allCategories, votes, onVote, onComplete }: VotingSessionViewProps) => {
    ...
    return (
        <div className={styles.container}>
            ...
            {!animatingState ? (
                ...
                <>
                    {/* Outgoing Slide */}
                    {renderSlide(animatingState.from, animatingState.direction === 'next' ? styles.exitNext : styles.exitPrev)}

                    {/* Incoming Slide */}
                    {renderSlide(animatingState.to, animatingState.direction === 'next' ? styles.enterNext : styles.enterPrev)}
                </>
            )}
        </div>
    );
