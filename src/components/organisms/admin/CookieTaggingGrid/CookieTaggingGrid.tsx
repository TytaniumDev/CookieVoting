...
export function CookieTaggingGrid({
    cookies,
    bakers,
    onUpdateCookies,
}: CookieTaggingGridProps) {
    const handleAssignBaker = useCallback((cookieId: string, bakerId: string) => {
    ...
    const handleRemoveAssignment = useCallback((cookieId: string) => {
        const updatedCookies = cookies.map((cookie) => {
            if (cookie.id === cookieId) {
