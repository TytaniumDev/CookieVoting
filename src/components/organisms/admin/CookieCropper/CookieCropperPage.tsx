...
export function CookieCropperPage({
  ...
  useEffect(() => {
    ...
    setRegions((prevRegions) => {
      ...
      const savedRegions: SliceRegion[] = existingCookies
        .filter(
          (c): c is typeof c & { cropRegion: NonNullable<typeof c.cropRegion> } => !!c.cropRegion,
        ) // Only those with crop data
        .map((c) => ({
          ...c.cropRegion,
          isSaved: true,
          savedImageId: c.id,
  ...
  const handleApplyGrid = useCallback(() => {
    ...
    const paddingPx = Math.round(
      (padding / 100) * Math.min(imageDimensions.width, imageDimensions.height),
    );
    ...
    return (
      <div className="flex flex-col w-full h-full bg-background overflow-hidden relative">
        <header className="flex items-center gap-4 px-6 py-4 bg-surface border-b border-white/10 flex-shrink-0 max-md:px-4 max-md:h-14">
          ...
          <h1 className="text-xl font-semibold text-white/90 flex-1 max-md:text-base">
            {displayTitle}
          </h1>
        </header>
