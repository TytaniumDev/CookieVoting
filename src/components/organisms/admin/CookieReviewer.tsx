...
export function CookieReviewer({ eventId, categoryId, onBack }: CookieReviewerProps) {
  ...
  if (loading)
    return <div className="p-8 text-center text-muted-foreground">Loading review interface...</div>;
      ...
      <header className="bg-card border-b p-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Review and adjust detected cookie boundaries
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={handleConfirm} disabled={globalLoading}>
            {globalLoading ? (
              'Processing...'
            ) : (
              <>
        ...
        >
          ...
          {containerSize.width > 0 &&
            crops.map((crop, i) => (
              ...
              >
                <span
                  className={`absolute -top-6 left-0 text-xs font-bold px-1.5 py-0.5 rounded shadow-sm select-none ${
                    selectedCropIndex === i
      ...
      {selectedCropIndex !== null ? (
        <div className="bg-card border-t p-4 flex justify-between items-center animate-in slide-in-from-bottom duration-200">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="secondary" size="sm" onClick={() => setSelectedCropIndex(null)}>
              Done
            </Button>
          </div>
        </div>
