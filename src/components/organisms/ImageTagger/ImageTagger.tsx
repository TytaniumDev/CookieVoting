  ...
  const handleAutoDetect = async () => {
    ...
    try {
      ...
      for (let i = 0; i < detected.length; i++) {
        ...
        const tooClose = cookies.some((existing) => {
          const distance = Math.sqrt(
            Math.pow(existing.x - detectedCookie.x, 2) + Math.pow(existing.y - detectedCookie.y, 2),
          );
      ...
      if (newCookies.length === 0) {
        setDetectionError(
          'All detected cookies were already tagged. Try manual tagging for any missed cookies.',
        );
      } else {
        setDetectionError(null);
      }
    ...
    <div className={styles.container}>
      <div className={styles.toolbar}>
        <div>
          <h3>Tagging Mode</h3>
          <p>
            Click on the image to add a number, or use auto-detect to find cookies automatically.
          </p>
          {loadingDetection && (
      ...
      <div className={styles.workspace}>
        ...
        >
          {(() => {
            ...
            const untaggedCookies: DetectedCookie[] = detectedCookies
              .map((detected) => {
                ...
                const isTagged = cookies.some((cookie) => {
                  const distance = Math.sqrt(
                    Math.pow(cookie.x - detected.x, 2) + Math.pow(cookie.y - detected.y, 2),
                  );
                  return distance < 5; // 5% threshold
                });
            ...
            detectedCookies.forEach((detected, index) => {
              const untagged = untaggedCookies.find(
                (uc) =>
                  uc.x === detected.x &&
                  uc.y === detected.y &&
        ...
        <div className={styles.sidebar}>
          ...
          {detectionError && <div className={styles.errorMessage}>{detectionError}</div>}
              ...
              .map((cookie) => (
                <div key={cookie.id} className={styles.item}>
                  <div className={styles.itemHeader}>
                    ...
                    <button onClick={() => removeCookie(cookie.id)} className={styles.deleteBtn}>
                      ×
                    </button>
                  </div>
                  ...
                  <input
                    ...
                    value={cookie.number}
                    onChange={(e) =>
                      updateCookie(cookie.id, { number: parseInt(e.target.value) || 0 })
                    }
                    className={styles.input}
                  />
                </div>
